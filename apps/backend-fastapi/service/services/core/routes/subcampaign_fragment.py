...
import strawberry
from .schemas import FragmentGet, FragmentPost, SubcampaignFragmentPost, SubcampaignFragmentGet, AuthPayload
...
def subcampaign_fragment_from_db(fragment: Dict[str, Any]) -> SubcampaignFragmentGet:
    return SubcampaignFragmentGet(id=fragment['id'], subcampaign_id=fragment['subcampaign_id'],
...
async def subcampaign_fragment_by_id(info: strawberry.Info[Context],
                                     subcampaign_fragment_id: int) -> SubcampaignFragmentGet:
...
async def create_subcampaign_fragment(info: strawberry.Info[Context],
                                      fg: SubcampaignFragmentPost) -> SubcampaignFragmentGet:
    user: AuthPayload = info.context.user()
    if user is None:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED)
