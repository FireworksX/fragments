...
from database import Session, Project, StreamFragment
...
async def create_stream_fragment(info: strawberry.Info[Context], stream_fragment_in: StreamFragmentPost) -> StreamFragmentGet:
...
async def stream_fragments_in_stream(info: strawberry.Info[Context], stream_id: int) -> List[StreamFragmentGet]:
    ...
    fragment: Fragment = await get_fragment_by_id_db(db, fg.fragment_id)
    if fragment is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Fragment does not exist")
...
async def update_stream_fragment(info: strawberry.Info[Context], fg: StreamFragmentPost) -> StreamFragmentGet:
    user: AuthPayload = await info.context.user()
    db: Session = info.context.session()

    stream_fragment: StreamFragmentGet = await get_stream_fragment_by_id_db(db, fg.id)
    if stream_fragment is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Stream Fragment does not exist")
