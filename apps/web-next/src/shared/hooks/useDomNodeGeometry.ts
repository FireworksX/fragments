...
export const useDomNodeGeometry = (nodeSelector: string, parentSelector: string) => {
  ...
  const layerNode = useWaitForElement(nodeSelector)
  const rootNode = useWaitForElement(parentSelector)
  ...
  }, [nodeSelector, parentSelector, runCalc])
