...
export const parseRawLayer = (schema: AnyZodObject, rawLayer: unknown, options?: Options) => {
  try {
    ...
    return Object.fromEntries(
      Object.entries(schema.shape).map(([key, schemaEntity]) => {
        ...
        const overrideValue = schemaMeta?.overridable !== false && overrideTarget ? overrideTarget?.[key] : null
