...
export const useBuilderLayout = () => {
  ...
  const layerInvoker = useLayerInvokerNew(
    ...
    ({ node, key, value }) => {
      switch (key) {
        ...
        case 'layerGap':
          ...
          break
        case 'padding':
          node.setPadding(value === graphState.mixed ? graphState.mixed : +value)
          break
        case 'paddingSide':
    ...
    ({ key, node }) => {
      switch (key) {
        case 'paddingSide':
          return {
            paddingTop: graphState.resolveValue?.(node, 'paddingTop'),
            paddingRight: graphState.resolveValue?.(node, 'paddingRight'),
            paddingBottom: graphState.resolveValue?.(node, 'paddingBottom'),
            paddingLeft: graphState.resolveValue?.(node, 'paddingLeft')
          }
      }
    }
  )
  ...
  return {
    ...
    padding: {
      ...
      mode: padding.value === graphState.mixed ? 'sides' : 'plain',
