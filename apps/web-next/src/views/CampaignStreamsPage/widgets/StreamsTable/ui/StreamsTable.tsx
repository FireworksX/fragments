...
import { times } from '@fragments/utils'
import { StatusBadge } from '@/shared/ui/StatusBadge'
import { Button } from '@/shared/ui/Button'
import { Chip } from '@/shared/ui/Chip'
import { Link } from '@/shared/ui/Link'
import { InputText } from '@/shared/ui/InputText'
import { useStreamsTable } from '@/views/CampaignStreamsPage/widgets/StreamsTable/hooks/useStreamsTable'
        ...
        <tbody className={styles.tbody}>
            ...
            <tr className={styles.row}>
              <td className={styles.cell} colSpan={6}>
                <Placeholder
                  ...
                  actions={<Button onClick={() => ref?.current?.createNew?.()}>Create Stream</Button>}
                />
              </td>
            </tr>
          )}
          ...
          {list.map(stream => (
            <tr key={stream.id} className={styles.row}>
              ...
              <td className={styles.cell}>{stream.weight}%</td>
              <td className={styles.cell}>
                <div className={styles.innerCell}>
                ...
                <div className={styles.innerCell}>
                  {stream.active ? (
                    <Button
                      ...
                      icon={<PauseIcon />}
                      onClick={() =>
                        updateStream({ campaignId: +campaignSlug, streamId: stream.id, campaignId: 1, active: false })
                      }
                    >
                      Pause
                    </Button>
                    ...
                    <Button
                      ...
                      icon={<PlayIcon />}
                      onClick={() =>
                        updateStream({ campaignId: +campaignSlug, streamId: stream.id, campaignId: 1, active: true })
                      }
                    >
                      Run
                    </Button>
