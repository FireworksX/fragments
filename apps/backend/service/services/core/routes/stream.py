...
def stream_db_to_stream(stream: Stream) -> StreamGet:
    ...
    for os_type_filter in stream.os_types_filter:
        filters.append(
            FilterOSTypeGet(type=FilterType.OSType, toggled=os_type_filter.toggled, os_type=os_type_filter.os_type))
    for device_type_filter in stream.device_types_filter:
        filters.append(FilterDeviceTypeGet(type=FilterType.DeviceType, toggled=device_type_filter.toggled,
                                           device_type=device_type_filter.device_type))
    ...
    for geo_location in stream.geo_locations_filter:
        filters.append(FilterGeoLocationGet(type=FilterType.GeoLocationType, toggled=geo_location.toggled,
                                            country=geo_location.country, region=geo_location.region,
                                            city=geo_location.city))
    for frame in stream.time_frames_filter:
        filters.append(
            FilterTimeFrameGet(type=FilterType.TimeFrameType, toggled=frame.toggled, from_time=frame.from_time,
                               to_time=frame.to_time))
...
async def streams_in_campaign(info: strawberry.Info[Context], campaign_id: int, active: Optional[bool] = None,
