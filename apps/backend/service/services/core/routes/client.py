...
from crud.area import get_areas_by_codes_db
...
async def client_areas_route(
    info: strawberry.Info[Context], area_codes: List[str]
) -> List[ClientAreaGet | None]:
    logger.info(f"Getting area variant for area codes {area_codes}")
    db: Session = info.context.session()
    ...
    areas: List[Area] = await get_areas_by_codes_db(db, area_codes)
    if len(areas) == 0:
        logger.debug(f"No areas found for client {client.id}")
        return []

    out: List[ClientAreaGet | None] = []
    for area in areas:
        if area.project_id != project.id:
            logger.warning(f"Area {area.area_code} does not belong to project {project.id}")
            raise HTTPException(
                ...
                for condition in condition_set.conditions:
                    ...
                    if condition.device_type_filters:
                        if client_info.device_type:
                            for device_type_filter in condition.device_type_filters:
                                if (
                                    int(client_info.device_type.value)
                        ...
                        if client_info.os_type:
                            for os_type_filter in condition.os_type_filters:
                                if int(client_info.os_type.value) == os_type_filter.os_type:
                                    logger.debug(
                                        f"OS type condition met: {client_info.os_type.value}"
                                    )
                                    condition_met = True
                                    break
                    ...
                    if condition.time_frame_filters:
                        ...
                        for time_frame_filter in condition.time_frame_filters:
                            if (
                                time_frame_filter.from_time
                                <= current_time
                                <= time_frame_filter.to_time
                            ):
                                logger.debug(
                                    f"Time frame condition met: current time {current_time}"
                                )
                                condition_met = True
                                break
        ...
        if (
            ...
            logger.debug(f'No suitable campaign found for area {area.area_code}')
            out.append(None)
            continue
        ...
        if best_campaign.feature_flag.rotation_type == int(RotationType.KEEP.value):
            logger.debug(
                f'Using KEEP rotation type - checking last viewed variant for area {area.area_code}'
            )
        ...
        if variantFragment is None:
            logger.debug(f'Selecting random variant based on weights for area {area.area_code}')
            active_variants = [
                v
        ...
        if variantFragment:
            logger.debug(
                f"Creating view history record for client {client.id} and variant {variantFragment.id} for area {area.area_code}"
            )
            await create_client_history_db(
                db=db,
                client_history=ClientHistoryPost(
            ...
            if (
                ...
                logger.debug(
                    f"Creating goal view history records for {len(variantFragment.fragment.fragment.linked_goals)} goals for area {area.area_code}"
                )
...
async def client_area_route(
    ...
    areas = await client_areas_route(info, [area_code])
    if len(areas) == 0:
        return None
    return areas[0]
