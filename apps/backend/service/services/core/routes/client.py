...
def client_db_to_client(client: Client, history: List[ClientHistory]) -> ClientGet:
    return ClientGet(
        id=client.id,
        last_visited=client.last_visited.isoformat() if client.last_visited else None,
...
async def create_client_history_route(
    info: strawberry.Info[Context], history: ClientHistoryInput
) -> ClientHistoryGet:
    db: Session = info.context.session()
    
    client: Client = await get_client_by_id_db(db, history.client_id)
    if client is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Client not found"
    ...
    history_entry: ClientHistory = await create_client_history_db(
        db,
        history.client_id,
        history.device_type,
        history.os_type,
        history.browser,
        history.language,
        history.screen_width,
        history.screen_height,
        history.country,
        history.region,
        history.city
    )
    
    return client_history_db_to_history(history_entry)
