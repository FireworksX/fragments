...
from services.core.routes.router import Mutation, Query
...
async def test_query_fragments():
    with patch(
        'services.core.routes.router.fragments_by_ids', new_callable=AsyncMock
    ) as mock_fragments:
...
async def test_query_campaign_by_id():
    with patch(
        'services.core.routes.router.campaign_by_id', new_callable=AsyncMock
    ) as mock_campaign:
...
async def test_query_campaigns_in_project():
    with patch(
        'services.core.routes.router.campaigns_in_project', new_callable=AsyncMock
    ) as mock_campaigns:
        mock_camp = Mock(spec=CampaignGet)
...
async def test_query_campaign_by_name():
    with patch(
        'services.core.routes.router.campaign_by_name', new_callable=AsyncMock
    ) as mock_campaign:
        mock_camp = Mock(spec=CampaignGet)
        mock_campaign.return_value = [mock_camp]
...
async def test_query_streams_in_campaign():
    with patch(
        'services.core.routes.router.streams_in_campaign', new_callable=AsyncMock
    ) as mock_streams:
        mock_strm = Mock(spec=StreamGet)
...
async def test_query_directory():
    with patch(
        'services.core.routes.router.get_directory', new_callable=AsyncMock
    ) as mock_directory:
...
async def test_query_client_fragment():
    with patch(
        'services.core.routes.router.get_client_fragment', new_callable=AsyncMock
    ) as mock_fragment:
...
async def test_create_project_goal():
    with patch(
        'services.core.routes.router.create_project_goal_route', new_callable=AsyncMock
    ) as mock_create_goal:
        ...
        mutation = Mutation()
        info = mock_info()
        result = await mutation.create_project_goal(
            info, goal=ProjectGoalPost(project_id=1, name='Test Goal', target_action='click')
        )

        assert result == mock_goal
        mock_create_goal.assert_called_once_with(
            info, ProjectGoalPost(project_id=1, name='Test Goal', target_action='click')
        )
...
async def test_update_project_goal():
    with patch(
        'services.core.routes.router.update_project_goal_route', new_callable=AsyncMock
    ) as mock_update_goal:
        mock_goal = Mock(spec=ProjectGoalGet)
        ...
        mutation = Mutation()
        info = mock_info()
        result = await mutation.update_project_goal(
            info, goal=ProjectGoalPatch(id=1, name='Updated Goal', target_action='submit')
        )

        assert result == mock_goal
        mock_update_goal.assert_called_once_with(
            info, ProjectGoalPatch(id=1, name='Updated Goal', target_action='submit')
        )
...
async def test_delete_project_goal():
    with patch(
        'services.core.routes.router.delete_project_goal_route', new_callable=AsyncMock
    ) as mock_delete_goal:
        mutation = Mutation()
        info = mock_info()
