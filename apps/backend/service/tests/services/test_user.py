...
from services.core.routes.user import signup, login
...
async def test_signup_successful():
    with patch('services.core.routes.user.get_user_by_email_db', new_callable=AsyncMock) as mock_get, \
            patch('services.core.routes.user.create_user_db', new_callable=AsyncMock) as mock_create:
...
async def test_signup_failed_to_create_user():
    with patch('services.core.routes.user.get_user_by_email_db', new_callable=AsyncMock) as mock_get, patch(
            'services.core.routes.user.create_user_db',
...
async def test_login_incorrect_password():
    with patch('services.core.routes.user.get_user_by_email_db', new_callable=AsyncMock) as mock_get, \
            patch('services.core.routes.user.verify_password', return_value=False) as mock_verify:
        mock_get.return_value = User(email='test@example.com', hashed_password='password')  # Existing user
...
async def test_login_successful():
    with patch('services.core.routes.user.get_user_by_email_db', new_callable=AsyncMock) as mock_get, \
            patch('services.core.routes.user.verify_password', return_value=True) as mock_verify, \
            patch('services.core.routes.user.create_access_token', return_value='access_token') as mock_create_access, \
            patch('services.core.routes.user.create_refresh_token',
                  return_value='refresh_token') as mock_create_refresh:
        mock_get.return_value = User(email='test@example.com', hashed_password='password')  # Existing user
