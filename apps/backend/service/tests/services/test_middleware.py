...
async def test_user_valid_token():
    with patch(
        'services.core.routes.middleware.get_user_by_email_db', new_callable=AsyncMock
    ) as mock_get_user:
        ...
        context.request = mock_request(
            headers={'Authorization': 'Bearer valid_token', 'Refresh': 'valid_refresh'}
        )
...
async def test_user_invalid_token():
    context = Context()
    context.request = mock_request(headers={'Authorization': 'Bearer invalid_token'})
...
async def test_project_valid_key():
    with patch(
        'services.core.routes.middleware.validate_project_public_api_key', new_callable=AsyncMock
    ) as mock_validate:
        mock_project = Mock(spec=Project)
        mock_validate.return_value = mock_project

        context = Context()
        context.request = mock_request(headers={'Authorization': 'Bearer valid_key'})
...
async def test_project_invalid_key():
    with patch(
        ...
        context = Context()
        context.request = mock_request(headers={'Authorization': 'Bearer invalid_key'})
