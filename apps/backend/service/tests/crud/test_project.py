...
from crud.project import (
    ...
    generate_api_key,
    generate_project_private_api_key,
    generate_project_public_api_key,
...
async def test_delete_project_public_api_key(mock_session):
    ...
    with pytest.raises(ValueError, match="Can't remove private key"):
        mock_filter.first.return_value = ProjectApiKey(
            id=key_id, project_id=project_id, is_private=True
        )
        await delete_project_public_api_key(mock_session, project_id, key_id)
...
async def test_create_project(mock_session):
    ...
    )  # Project, ApiKey, ProjectMemberRole, FilesystemDirectory
...
async def test_project_goals(mock_session):
    ...
    mock_goal = ProjectGoal(
        id=goal_id, project_id=project_id, name=name, target_action=target_action
    )
    mock_session.refresh.side_effect = lambda x: None
