...
export const useLayerSize = (layerKey: LinkKey) => {
  ...
  //   instanceManager
  // );
  const [width] = useLayerValue(layerKey, "width", 0);
  const [height] = useLayerValue(layerKey, "height", 0);
  const [widthType] = useLayerValue(layerKey, "widthType", sizing.Fixed);
  const [heightType] = useLayerValue(layerKey, "heightType", sizing.Fixed);
