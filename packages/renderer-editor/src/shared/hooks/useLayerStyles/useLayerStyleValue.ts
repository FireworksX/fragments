...
export const useLayerStyleValue = (layerKey: LinkKey, fieldKey: string) => {
  const { manager } = use(FragmentContext);
  const [value] = useLayerVariableValue(layerKey, fieldKey);
  const value$ = useSpringValue(value, {
    onPause: () => {
      console.log("pause", layerKey, fieldKey);
    },
    onStart: () => {
      console.log("start", layerKey, fieldKey);
    },
    onRest: () => {
      console.log("onRest", layerKey, fieldKey);
    },
    onResume: () => {
      console.log("onResume", layerKey, fieldKey);
    },
  });
